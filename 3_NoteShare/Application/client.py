    ...
    def send_action_message(self, data=None):
        try:
            ...
            if server_message["success"] and server_message["message"] == "Your note has been uploaded!":
                Message.sending_file(filename=server_message["filename"], client_socket=self.__client_socket, 
                ...
                Message.receiving_file(filename=server_message["filename"], client_socket=self.__client_socket,
                                       network_buffer_size=self.__network_buffer_size)
        except:
            server_message = {"success":False, "message":"Server is temporarily unavailable. Try again later!"}
        finally:
