...
parser.add_argument("channel", help = "Please, enter the specific channel that you want to subscribe to.")
parser.add_argument("name", help = "Please, enter your username.")
args = parser.parse_args()

def posting_channel(args_channel, args_ip, args_post_port, args_name, context):
    if args_channel.upper() != "ALL":
        while True:
            message = input()
            if message:
                ...
                post_message = f"{args_name},{current_time},{message},{args_channel}"
                post_socket.send_string(post_message)
                post_socket.recv_string()
                post_socket.close()

def publishing_channel(args_channel, args_ip, args_pub_port, context):
    while True:
        ...
        list_of_pub_messages = pub_message.split(",")
        if args_channel.upper() == "ALL" or list_of_pub_messages[3] == args_channel:
            pub_message = f"{list_of_pub_messages[0]}: {list_of_pub_messages[2]} ({list_of_pub_messages[1]})"
...
if args.post_port.isnumeric() and 1024 <= int(args.post_port) <= 65535 and args.pub_port.isnumeric() and 1024 <= int(args.pub_port) <= 65535:
    context = zmq.Context()
    post_thread = threading.Thread(target = posting_channel, args = [args.channel, args.ip, args.post_port, args.name, context])
    pub_thread = threading.Thread(target = publishing_channel, args = [args.channel, args.ip, args.pub_port, context])
    post_thread.start()
    pub_thread.start()
    post_thread.join()
    pub_thread.join()
